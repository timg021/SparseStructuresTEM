...
Fast Fourier/Cosine/Sine Transform
    ...
    table       :use
functions
    cdft2d_float: Complex Discrete Fourier Transform
    rdft2d_float: Real Discrete Fourier Transform
    ddct2d_float: Discrete Cosine Transform
    ddst2d_float: Discrete Sine Transform
function prototypes
...
-------- Complex DFT (Discrete Fourier Transform) --------
    ...
    [usage]
        <case1>
            ...
            cdft2d_float(n1, 2*n2, 1, a, t, ip, w);
        <case2>
            ip[0] = 0; // first time only
            cdft2d_float(n1, 2*n2, -1, a, t, ip, w);
    [parameters]
...
void cdft2d_float(int n1, int n2, int isgn, float **a, float *t, 
    int *ip, float *w)
{
    void makewt_float(int nw, int *ip, float *w);
    void cdft_float(int n, int isgn, float *a, int *ip, float *w);
    void cdft2d_sub(int n1, int n2, int isgn, float **a, float *t, 
        int *ip, float *w);
#ifdef USE_FFT2D_THREADS
    void xdft2d0_subth(int n1, int n2, int icr, int isgn, float **a, 
        int *ip, float *w);
    void cdft2d_subth(int n1, int n2, int isgn, float **a, float *t, 
        int *ip, float *w);
...
void rdft2d_float(int n1, int n2, int isgn, float **a, float *t, 
    int *ip, float *w)
{
    void makewt_float(int nw, int *ip, float *w);
    void makect_float(int nc, int *ip, float *c);
    void rdft_float(int n, int isgn, float *a, int *ip, float *w);
    void cdft2d_sub(int n1, int n2, int isgn, float **a, float *t, 
        int *ip, float *w);
    ...
    if (n > (nw << 2)) {
        nw = n >> 2;
        makewt_float(nw, ip, w);
    }
    nc = ip[1];
    if (n2 > (nc << 2)) {
        nc = n2 >> 2;
        makect_float(nc, ip, w + nw);
    }
    itnull = 0;
    if (t == NULL) {
        itnull = 1;
...
void rdft2dsort_float(int n1, int n2, int isgn, float **a)
{
    int n1h, i;
    float x, y;
...
void ddct2d_float(int n1, int n2, int isgn, float **a, float *t, 
    int *ip, float *w)
{
    void makewt_float(int nw, int *ip, float *w);
    void makect_float(int nc, int *ip, float *c);
    void ddct_float(int n, int isgn, float *a, int *ip, float *w);
    void ddxt2d_sub(int n1, int n2, int ics, int isgn, float **a, 
        float *t, int *ip, float *w);
#ifdef USE_FFT2D_THREADS
    void ddxt2d0_subth(int n1, int n2, int ics, int isgn, float **a, 
        int *ip, float *w);
    void ddxt2d_subth(int n1, int n2, int ics, int isgn, float **a, 
        float *t, int *ip, float *w);
#endif /* USE_FFT2D_THREADS */
    int n, nw, nc, itnull, nthread, nt, i;
...
void ddst2d_float(int n1, int n2, int isgn, float **a, float *t, 
    int *ip, float *w)
{
    void makewt_float(int nw, int *ip, float *w);
    void makect_float(int nc, int *ip, float *c);
    void ddst_float(int n, int isgn, float *a, int *ip, float *w);
    void ddxt2d_sub(int n1, int n2, int ics, int isgn, float **a, 
        float *t, int *ip, float *w);
#ifdef USE_FFT2D_THREADS
    void ddxt2d0_subth(int n1, int n2, int ics, int isgn, float **a, 
...
static void cdft2d_sub(int n1, int n2, int isgn, float **a, float *t, 
    int *ip, float *w)
{
    void cdft_float(int n, int isgn, float *a, int *ip, float *w);
    int i, j;
    ...
    if (n2 > 4) {
        for (j = 0; j < n2; j += 8) {
            ...
            cdft_float(2 * n1, isgn, t, ip, w);
            cdft_float(2 * n1, isgn, &t[2 * n1], ip, w);
            cdft_float(2 * n1, isgn, &t[4 * n1], ip, w);
            cdft_float(2 * n1, isgn, &t[6 * n1], ip, w);
            for (i = 0; i < n1; i++) {
                a[i][j] = t[2 * i];
                a[i][j + 1] = t[2 * i + 1];
                a[i][j + 2] = t[2 * n1 + 2 * i];
...
static void rdft2d_sub(int n1, int n2, int isgn, float **a)
{
    int n1h, i, j;
    float xi;
...
static void ddxt2d_sub(int n1, int n2, int ics, int isgn, float **a, 
    float *t, int *ip, float *w)
{
    void ddct_float(int n, int isgn, float *a, int *ip, float *w);
    void ddst_float(int n, int isgn, float *a, int *ip, float *w);
    ...
    if (n2 > 2) {
        for (j = 0; j < n2; j += 4) {
            ...
            if (ics == 0) {
                ddct_float(n1, isgn, t, ip, w);
                ddct_float(n1, isgn, &t[n1], ip, w);
                ddct_float(n1, isgn, &t[2 * n1], ip, w);
                ddct_float(n1, isgn, &t[3 * n1], ip, w);
            } else {
                ddst_float(n1, isgn, t, ip, w);
                ddst_float(n1, isgn, &t[n1], ip, w);
                ddst_float(n1, isgn, &t[2 * n1], ip, w);
                ddst_float(n1, isgn, &t[3 * n1], ip, w);
            }
            for (i = 0; i < n1; i++) {
                a[i][j] = t[i];
                a[i][j + 1] = t[n1 + i];
...
static void xdft2d0_subth(int n1, int n2, int icr, int isgn, float **a, 
    int *ip, float *w)
{
...
static void cdft2d_subth(int n1, int n2, int isgn, float **a, float *t, 
    int *ip, float *w)
{
...
static void ddxt2d0_subth(int n1, int n2, int ics, int isgn, float **a, 
    int *ip, float *w)
{
...
static void ddxt2d_subth(int n1, int n2, int ics, int isgn, float **a, 
    float *t, int *ip, float *w)
{
