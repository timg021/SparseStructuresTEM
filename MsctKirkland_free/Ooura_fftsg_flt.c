...
Fast Fourier/Cosine/Sine Transform
    ...
    table       :use
functions
    cdft_float: Complex Discrete Fourier Transform
    rdft_float: Real Discrete Fourier Transform
    ddct_float: Discrete Cosine Transform
    ddst_float: Discrete Sine Transform
    dfct_float: Cosine Transform of rdft_float (Real Symmetric DFT)
    dfst_float: Sine Transform of rdft_float (Real Anti-symmetric DFT)
function prototypes
...
-------- Cosine Transform of rdft_float (Real Symmetric DFT) --------
    [definition]
        C[k] = sum_j=0^n a[j]*cos(pi*j*k/n), 0<=k<=n
    [usage]
        ip[0] = 0; // first time only
        dfct_float(n, a, t, ip, w);
    [parameters]
        n              :data length - 1 (int)
                        n >= 2, n = power of 2
...
-------- Sine Transform of rdft_float (Real Anti-symmetric DFT) --------
    [definition]
        S[k] = sum_j=1^n-1 a[j]*sin(pi*j*k/n), 0<k<n
    [usage]
        ip[0] = 0; // first time only
        dfst_float(n, a, t, ip, w);
    [parameters]
        n              :data length + 1 (int)
                        n >= 2, n = power of 2
        a[0...n-1]     :input/output data (float *)
...
void dfct_float(int n, float *a, float *t, int *ip, float *w)
{
    void makewt_float(int nw, int *ip, float *w);
    void makect_float(int nc, int *ip, float *c);
    void cftfsub(int n, float *a, int *ip, int nw, float *w);
...
void dfst_float(int n, float *a, float *t, int *ip, float *w)
{
    void makewt_float(int nw, int *ip, float *w);
    void makect_float(int nc, int *ip, float *c);
    void cftfsub(int n, float *a, int *ip, int nw, float *w);
...
void makewt_float(int nw, int *ip, float *w)
{
    void makeipt_float(int nw, int *ip);
    int j, nwh, nw0, nw1;
    ...
    if (nw > 2) {
        ...
        w[1] = wn4r;
        if (nwh == 4) {
            w[2] = (float) cos(delta * 2);
            w[3] = (float) sin(delta * 2);
        } else if (nwh > 4) {
            makeipt_float(nw, ip);
            w[2] = (float) (0.5 / cos(delta * 2));
            w[3] = (float) (0.5 / cos(delta * 6));
            for (j = 4; j < nwh; j += 4) {
                w[j] = (float) cos(delta * j);
                w[j + 1] = (float) sin(delta * j);
                w[j + 2] = (float) cos(3 * delta * j);
                w[j + 3] = (float) -sin(3 * delta * j);
            }
        }
        nw0 = 0;
        while (nwh > 2) {
...
#define cdft_thread_create(thp,func,argp) { \
    if (pthread_create(thp, NULL, func, (void *) argp) != 0) { \
        fprintf(stderr, "cdft_float thread error\n"); \
        exit(1); \
    } \
}
