...
Fast Fourier/Cosine/Sine Transform
    ...
    table       :use
functions
    cdft2d_double: Complex Discrete Fourier Transform
    rdft2d_double: Real Discrete Fourier Transform
    ddct2d_double: Discrete Cosine Transform
    ddst2d_double: Discrete Sine Transform
function prototypes
...
-------- Complex DFT (Discrete Fourier Transform) --------
    ...
    [usage]
        <case1>
            ...
            cdft2d_double(n1, 2*n2, 1, a, t, ip, w);
        <case2>
            ip[0] = 0; // first time only
            cdft2d_double(n1, 2*n2, -1, a, t, ip, w);
    [parameters]
...
-------- Real DFT / Inverse of Real DFT --------
    ...
    [usage]
        <case1>
            ip[0] = 0; // first time only
            rdft2d_double(n1, n2, 1, a, t, ip, w);
        <case2>
            ip[0] = 0; // first time only
            rdft2d_double(n1, n2, -1, a, t, ip, w);
    [parameters]
...
-------- DCT (Discrete Cosine Transform) / Inverse of DCT --------
    ...
    [usage]
        <case1>
            ip[0] = 0; // first time only
            ddct2d_double(n1, n2, 1, a, t, ip, w);
        <case2>
            ip[0] = 0; // first time only
            ddct2d_double(n1, n2, -1, a, t, ip, w);
    [parameters]
...
-------- DST (Discrete Sine Transform) / Inverse of DST --------
    ...
    [usage]
        <case1>
            ip[0] = 0; // first time only
            ddst2d_double(n1, n2, 1, a, t, ip, w);
        <case2>
            ip[0] = 0; // first time only
            ddst2d_double(n1, n2, -1, a, t, ip, w);
    [parameters]
...
void cdft2d_double(int n1, int n2, int isgn, double **a, double *t, 
    int *ip, double *w)
{
    void makewt_double(int nw, int *ip, double *w);
    void cdft_double(int n, int isgn, double *a, int *ip, double *w);
...
void rdft2d_double(int n1, int n2, int isgn, double **a, double *t, 
    int *ip, double *w)
{
    void makewt_double(int nw, int *ip, double *w);
    void makect_double(int nc, int *ip, double *c);
    void rdft_double(int n, int isgn, double *a, int *ip, double *w);
    void cdft2d_sub(int n1, int n2, int isgn, double **a, double *t, 
        int *ip, double *w);
    ...
    if (n > (nw << 2)) {
        nw = n >> 2;
        makewt_double(nw, ip, w);
    }
    nc = ip[1];
    if (n2 > (nc << 2)) {
        nc = n2 >> 2;
        makect_double(nc, ip, w + nw);
    }
    itnull = 0;
    if (t == NULL) {
        itnull = 1;
...
void rdft2dsort_double(int n1, int n2, int isgn, double **a)
{
    int n1h, i;
    double x, y;
...
void ddct2d_double(int n1, int n2, int isgn, double **a, double *t, 
    int *ip, double *w)
{
    void makewt_double(int nw, int *ip, double *w);
    void makect_double(int nc, int *ip, double *c);
    void ddct_double(int n, int isgn, double *a, int *ip, double *w);
...
void ddst2d_double(int n1, int n2, int isgn, double **a, double *t, 
    int *ip, double *w)
{
    void makewt_double(int nw, int *ip, double *w);
    void makect_double(int nc, int *ip, double *c);
    void ddst_double(int n, int isgn, double *a, int *ip, double *w);
    void ddxt2d_sub(int n1, int n2, int ics, int isgn, double **a, 
        double *t, int *ip, double *w);
#ifdef USE_FFT2D_THREADS
    void ddxt2d0_subth(int n1, int n2, int ics, int isgn, double **a, 
...
static void cdft2d_sub(int n1, int n2, int isgn, double **a, double *t, 
    int *ip, double *w)
{
    void cdft_double(int n, int isgn, double *a, int *ip, double *w);
    int i, j;
    ...
    if (n2 > 4) {
        for (j = 0; j < n2; j += 8) {
            ...
            cdft_double(2 * n1, isgn, t, ip, w);
            cdft_double(2 * n1, isgn, &t[2 * n1], ip, w);
            cdft_double(2 * n1, isgn, &t[4 * n1], ip, w);
            cdft_double(2 * n1, isgn, &t[6 * n1], ip, w);
            for (i = 0; i < n1; i++) {
                a[i][j] = t[2 * i];
                a[i][j + 1] = t[2 * i + 1];
                a[i][j + 2] = t[2 * n1 + 2 * i];
...
static void rdft2d_sub(int n1, int n2, int isgn, double **a)
{
    int n1h, i, j;
    double xi;
...
static void ddxt2d_sub(int n1, int n2, int ics, int isgn, double **a, 
    double *t, int *ip, double *w)
{
    void ddct_double(int n, int isgn, double *a, int *ip, double *w);
    void ddst_double(int n, int isgn, double *a, int *ip, double *w);
    ...
    if (n2 > 2) {
        for (j = 0; j < n2; j += 4) {
            ...
            if (ics == 0) {
                ddct_double(n1, isgn, t, ip, w);
                ddct_double(n1, isgn, &t[n1], ip, w);
                ddct_double(n1, isgn, &t[2 * n1], ip, w);
                ddct_double(n1, isgn, &t[3 * n1], ip, w);
            } else {
                ddst_double(n1, isgn, t, ip, w);
                ddst_double(n1, isgn, &t[n1], ip, w);
                ddst_double(n1, isgn, &t[2 * n1], ip, w);
                ddst_double(n1, isgn, &t[3 * n1], ip, w);
            }
            for (i = 0; i < n1; i++) {
                a[i][j] = t[i];
                a[i][j + 1] = t[n1 + i];
...
static void xdft2d0_subth(int n1, int n2, int icr, int isgn, double **a, 
    int *ip, double *w)
{
...
static void cdft2d_subth(int n1, int n2, int isgn, double **a, double *t, 
    int *ip, double *w)
{
...
static void ddxt2d0_subth(int n1, int n2, int ics, int isgn, double **a, 
    int *ip, double *w)
{
...
static void ddxt2d_subth(int n1, int n2, int ics, int isgn, double **a, 
    double *t, int *ip, double *w)
{
