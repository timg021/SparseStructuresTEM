...
Fast Fourier/Cosine/Sine Transform
    ...
    table       :use
functions
    cdft_double: Complex Discrete Fourier Transform
    rdft_double: Real Discrete Fourier Transform
    ddct_double: Discrete Cosine Transform
    ddst_double: Discrete Sine Transform
    dfct_double: Cosine Transform of rdft_double (Real Symmetric DFT)
    dfst_double: Sine Transform of rdft_double (Real Anti-symmetric DFT)
function prototypes
...
-------- Cosine Transform of rdft_double (Real Symmetric DFT) --------
    [definition]
        C[k] = sum_j=0^n a[j]*cos(pi*j*k/n), 0<=k<=n
    [usage]
        ip[0] = 0; // first time only
        dfct_double(n, a, t, ip, w);
    [parameters]
        n              :data length - 1 (int)
                        n >= 2, n = power of 2
...
-------- Sine Transform of rdft_double (Real Anti-symmetric DFT) --------
    [definition]
        S[k] = sum_j=1^n-1 a[j]*sin(pi*j*k/n), 0<k<n
    [usage]
        ip[0] = 0; // first time only
        dfst_double(n, a, t, ip, w);
    [parameters]
        n              :data length + 1 (int)
                        n >= 2, n = power of 2
        a[0...n-1]     :input/output data (double *)
...
void dfct_double(int n, double *a, double *t, int *ip, double *w)
{
    void makewt_double(int nw, int *ip, double *w);
    void makect_double(int nc, int *ip, double *c);
    void cftfsub(int n, double *a, int *ip, int nw, double *w);
...
void dfst_double(int n, double *a, double *t, int *ip, double *w)
{
    void makewt_double(int nw, int *ip, double *w);
    void makect_double(int nc, int *ip, double *c);
    void cftfsub(int n, double *a, int *ip, int nw, double *w);
...
void makewt_double(int nw, int *ip, double *w)
{
    void makeipt_double(int nw, int *ip);
    int j, nwh, nw0, nw1;
...
#define cdft_thread_create(thp,func,argp) { \
    if (pthread_create(thp, NULL, func, (void *) argp) != 0) { \
        fprintf(stderr, "cdft_double thread error\n"); \
        exit(1); \
    } \
}
